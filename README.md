...
“题目池”中的“一键自动运行完整流程”会持续维持 3 个活动 Pair。系统按 `0–1：Feature：Bug 修复 = 7：7：10` 的滚动缺口选择题目，旧 0–1 题排在新题之后，并在选用时再次拦截重复标题和高相似题面。同一个 0–1 根项目最多生成 3 个 Feature 候选，达到上限后自动创建新的 0–1 项目。系统自动创建仓库、准备并启动 A/B、执行 Docker 验收、根据真实改动与验证结果复评难度、依次录制两侧真实操作、生成并默认确认 GSB；任一 Pair 完成后会自动补位。0–1 和 Feature 的实际难度必须为困难或地狱，Bug 修复允许中等、困难或地狱；低于对应准入线的 Pair 会停止并释放名额。没有可用题目时才启动自动出题和准入复核。停止自动运行只停止后续推进和补位，不会强制中断已经启动的开发容器。
