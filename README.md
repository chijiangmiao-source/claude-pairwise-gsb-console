...
- 每个有效 Claude Session 只发送一次原题。API Error（包括 429、504、证书与连接错误）、异常退出、无产出超时以及收尾失败都会使当前尝试作废；系统先停止容器、导出并校验轨迹，再从共同基线启动全新容器、工作区和 Session 重跑，不发送“继续”。429 不计入开发失败额度，504 计入一次；轨迹复制或校验失败时保留旧容器。累计三次失败后退役当前 Pair，并自动换一道困难或地狱题启动新 Pair。
- Git、Docker、SQLite、轨迹导出和录像由系统代码执行，不把实际操作结果交给模型虚构。

页面中的“验收与录像”把 A/B Docker 验收和 720p 操作录像合并展示，可直接播放并核对录像提交与最终提交是否一致。通过技术检查的录像按用户授权默认审核通过，人工抽查发现问题后可重新录制。GSB 生成或应用复检建议后同样默认确认，仍可在“复核与人工确认”中检索、复检、编辑和重新确认。“导出轮次”支持资料筛选、提交前检查、批量复检、Excel 复核副本、隐藏/恢复，以及通过 Chrome 小助手向本期 SOLO-QA Pair-wise 表单提交、返修和同步质检状态；页头、勾选批量操作和每条已提交记录均提供状态同步入口。三个页面的筛选、页码和每页条数会保存在 URL 中。
