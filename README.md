...
- 每个有效 Claude Session 只发送一次原题。A/B 容器都准备完成后先向 A 发送，默认 30 秒后再向 B 发送同一题面；两侧的告警和停止计时都从各自实际发送时刻开始。API Error（包括 429、504、证书与连接错误）、异常退出、无产出超时以及收尾失败都会使当前尝试作废；系统先停止容器、导出并校验轨迹，再从共同基线启动全新容器、工作区和 Session 重跑，不发送“继续”。429 不计入开发失败额度，504 计入一次；轨迹复制或校验失败时保留旧容器。累计三次失败后退役当前 Pair，并自动换一道困难或地狱题启动新 Pair。
...
“题目池”中的“一键自动运行完整流程”会持续维持 3 个活动 Pair。系统优先使用现有已通过准入的困难或地狱题，自动创建仓库、准备并启动 A/B、执行 Docker 验收、根据真实改动与验证结果复评难度、依次录制两侧真实操作、生成并默认确认 GSB；任一 Pair 完成后会自动补位。实际难度降为简单或中等的 Pair 会停止并释放名额。没有可用题目时才启动自动出题和准入复核。停止自动运行只停止后续推进和补位，不会强制中断已经启动的开发容器。
