...
- 每个有效 Claude Session 只发送一次原题。API Error（包括 429、504、证书与连接错误）、异常退出、无产出超时以及收尾失败都会使当前尝试作废；系统先停止容器、导出并校验轨迹，再从共同基线启动全新容器、工作区和 Session 重跑，不发送“继续”。轨迹复制或校验失败时保留旧容器。连续三次失败后退役当前 Pair，并自动换一道困难或地狱题启动新 Pair。
- Git、Docker、SQLite、轨迹导出和录像由系统代码执行，不把实际操作结果交给模型虚构。

页面中的“验收与录像”把 A/B Docker 验收和 720p 操作录像合并展示，可直接播放并核对录像提交与最终提交是否一致。通过技术检查的录像按用户授权默认审核通过，人工抽查发现问题后可重新录制。GSB 生成或应用复检建议后同样默认确认，仍可在“复核与人工确认”中检索、复检、编辑和重新确认。“导出轮次”支持资料筛选、提交前检查、批量复检、Excel 复核副本、隐藏/恢复，以及通过 Chrome 小助手向本期 SOLO-QA Pair-wise 表单提交、返修和同步质检状态。三个页面的筛选、页码和每页条数会保存在 URL 中。

“题目池”中的“一键自动运行完整流程”会持续维持 3 个活动 Pair。系统优先使用现有已通过准入的困难或地狱题，自动创建仓库、准备并启动 A/B、执行 Docker 验收、依次录制两侧真实操作、生成并默认确认 GSB；任一 Pair 完成后会自动补位。没有可用题目时才启动自动出题和准入复核。停止自动运行只停止后续推进和补位，不会强制中断已经启动的开发容器。

录像统一使用独立 Chrome 的网页视口录制，输出通用的 H.264 MP4，只保存 `1280×720` 页面内容，并持续显示页面内鼠标指针和点击圆点。点击“一键启动并录制”或“不合格并重新录制”后，系统会启动最终提交对应的 Docker Compose、识别浏览器入口、打开 Chrome 并自动录像；Swagger 文档页会隐藏自动生成的 `/openapi.json` 链接，再按“展开业务接口、Try it out、填写有效参数、Execute、展示成功响应”的真实流程操作。普通前端录像也必须检测到页面点击和同源成功接口请求。只有 A/B 当前最终提交都通过 Docker 清洁验收后才允许录像和生成 GSB；缺少 Dockerfile、Compose 或启动失败时，失败 Arm 会从共同 `main` 基线开启新会话重跑，达到三次上限后整组失败并自动补位。没有完成真实功能流程的录像即使分辨率和时长正确也不会替换当前合格录像。历次录像保留在历史记录中，重录失败不会覆盖当前合格录像，重录成功会重新绑定提交证据并使旧 GSB 进入重新评价状态。
