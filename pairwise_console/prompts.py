...
只生成实际修复难度达到困难或地狱的候选。不设最低修复时间，但熟悉项目的开发者应能在 120 分钟内完成；合理修复至少涉及 20 行有效生产代码。改常量、恢复一行条件、补一个显而易见的校验或照现有测试抄答案的缺陷不合格。API 限流、证书、网络临时中断和机器资源不足不是产品 Bug。找不到可信候选时返回空 candidates，禁止编造。
...
只在以下条件全部满足时 accepted=true：候选已经由两次清洁 Docker 环境稳定复现，题面中的事实与复现证据一致；实际修复难度至少为困难；不设最低修复时间，但熟悉项目的开发者预计不得超过 120 分钟；合理修复预计至少涉及 20 行有效生产代码；题面能通过真实业务入口复现和验收，同时没有透露根因、内部文件名、函数名、代码行号、修改位置、补丁策略、建议算法或实现答案。
...
estimatedChangedLines 和 estimatedChangedFiles 评估正确修复的合理生产代码改动量。若证据不足以支持困难级别或至少 20 行修复，或预计修复超过 120 分钟，必须拒绝，不能按题面自报难度通过。
...
def seeded_bug_prompt(original_task: str, project_summary: str,
    ...
    return f"""你要为一个隔离的 A/B 修复基准制作带缺陷的初始仓库。直接检查并修改当前目录里的生产代码，只引入一个真实、稳定、可由现有业务入口触发的 Bug；不设最低修复时间，但熟悉项目的开发者应能在 120 分钟内完成。难度必须达到困难或地狱，合理修复至少需要 20 行有效生产代码，并且需要理解跨模块状态、持久化、并发、异常恢复、精确算法或前后端契约中的至少一项关键关系。

只改实现该缺陷所必需的生产代码。注入变更本身必须包含至少 12 行有意义的生产逻辑改动，并形成需要理解状态、算法、并发、持久化、异常恢复或跨层契约才能修复的缺陷；不得通过注释、格式化、重复代码或无关重写凑行数。选定候选、动手修改之前，先反推开发者面对缺陷仓库时最小且正确的通用修法；只有该修法本身确实需要至少 20 行有效生产逻辑时才采用，否则必须换候选。优先破坏一个分布在两个以上状态或模块中的既有不变量，让正确修复必须重新贯通状态模型、持久化或接口契约。若通过删除或回退本次注入、加一把锁、恢复排序、补一个字段或条件、替换公式或常量、修正单个坐标映射等少量改动就能完整修好，即使注入代码很多也不合格。不要新增或修改测试、README、注释、Dockerfile、Compose、verify 脚本、锁文件和依赖清单，也不要留下 TODO、故障开关、补丁说明、答案提示或故意报错文本。不得硬编码只针对一个示例的分支。修改后原有启动、健康检查和 verify 仍应通过，缺陷只在题面给出的特定业务场景中显现。

动手前先反向设计正确修复的最小方案：如果熟悉仓库的人能够通过删除本次新增分支、恢复输入顺序、携带一个原始索引、补全一个集合或键、改一处比较，或在单个位置做少于 20 行的修改解决，就放弃该想法并另选缺陷。除非需要实质重写复杂核心算法，否则正确修复应跨至少两个生产模块或文件，并同时恢复两个以上可观察业务契约。不要为了证明注入成功而安装依赖、下载包或运行 Docker；系统会在你返回后独立执行清洁 Compose 预检。只运行仓库现成且无需安装的快速检查。
...
只在同时满足以下条件时 accepted=true：Bug 可由题面描述的真实业务入口稳定触发；修复需要理解关键状态、算法、并发、持久化、异常恢复或跨层契约，实际难度至少为困难；不设最低修复时间，但预计不得超过 120 分钟；合理修复预计至少涉及 20 行有效生产代码，可以只改一个核心文件，但不能只是改常量、恢复一行条件或照题面抄答案；注入 diff 至少要有 12 行有意义的生产逻辑变更，不能是单行公式、坐标映射、常量或条件替换，也不能靠无关重写凑行数；题面没有出现内部文件名、函数名、修改位置、根因、补丁策略或其他解法提示；题面没有出现 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E、真实点击脚本等相关要求或禁用说明，自动化验收使用代码测试、构建检查和 API/HTTP 冒烟；注入本身不是显眼的故障开关、特例硬编码、故意抛错或破坏启动。
