...
优先寻找复杂状态、并发、持久化、兼容性、权限、性能边界或异常恢复中的真实缺陷。候选必须给出明确前置条件、逐步操作、预期结果、预计实际结果、代码位置和修复复杂度。每个候选还必须提供 reproductionCommands：每项只写 docker compose 的子参数，例如 ["exec","-T","api","pytest","tests/test_x.py::test_case"] 或 ["run","--rm","verify",...]；系统会统一补上 docker compose、项目名和 Compose 文件，并在两次全新启动中执行。命令必须只读取或测试当前产物，不能修改源码、宿主设置、Git 或其他项目，也不得安装或运行 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化或浏览器 E2E。优先使用代码测试、构建检查和 API/HTTP 冒烟。expectedExitCode 与 expectedOutputContains 要能客观判断该缺陷是否复现。没有现成的可重复命令就不要输出该候选。
...
开发者只能看到业务现象和可复现证据。题面不得透露根因，不得写内部文件名、函数名、代码行号、修改位置、补丁策略、建议算法或实现答案；候选中的 sourcePaths 和内部分析只供你核对，不能出现在 prompt。公开接口、页面控件、输入数据、状态码和通过真实业务入口执行的命令可以保留。若现有复现命令直接导入内部函数或读取源码，应把它改写成业务场景说明，并要求修复后通过代码测试、构建检查、HTTP API 或仓库已有的非浏览器公开验收入口验证，不能把内部调用链发给开发者。

根据这个 Bug 自身的因果关系自然组织文字。不要套用固定开头、固定段落顺序或固定结尾；不要使用“前置条件：”“复现步骤：”“实际结果：”“预期结果：”四段结构，也不要写“请修复该问题，保留现有 Docker Compose 启动与验收链路，并补充覆盖复现路径的自动化验收”。可以使用自然段；只有确实有助于执行时才使用短列表。回归验收要说明真实需要执行的场景和结果，不能只写“补充测试”。自动化验收只允许代码测试、构建检查和 API/HTTP 冒烟，不得要求 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E 或真实点击脚本；页面真实操作与录像由交付流程另行完成。
...
公开 API 路径、页面控件、输入数据、状态码和业务错误可以作为复现锚点，不算答案泄露。直接告诉开发者哪个内部判断错了、应改哪个函数、采用线程池、缓存或动态规划等具体修法，或把只读源码和直接导入内部求解器当成主要验收，均视为 answerLeak=true。公开题面不得要求 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化或浏览器 E2E；自动化验收只能使用代码测试、构建检查和 API/HTTP 冒烟。不要因为题面长、数字多或业务名复杂就判为困难。
...
同时生成自然中文修复题面。题面说明真实触发条件、操作、实际现象、正确行为和回归验收，但不能透露根因、修改位置、实现方案或你改过哪些文件；不要套用“前置条件/复现步骤/实际结果/预期结果”四段模板。题面须保留现有 Docker Compose 启动方式，并自然写明“自动化验收”的具体场景和应核对结果；自动化部分只允许代码测试、构建检查和 API/HTTP 冒烟，不得要求 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E 或真实点击脚本。页面现象可以作为业务结果描述，真实点击与录像由系统后续独立完成。最终题面必须同时出现“自动化”和“验收”或“测试”。不要加入仓库内已有测试可以直接抄出的答案。
