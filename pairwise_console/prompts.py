...
DIFFICULTY_RULES = """0–1 和 Feature 只有困难或地狱可进入 A/B。Bug 修复优先困难或地狱；实际为中等时，只有合理修复预计至少涉及 50 行有效生产代码才可进入，简单 Bug 仍拒绝。困难必须整合多个模块或系统约束，做关键设计取舍，并处理复杂状态、兼容性、权限、并发、性能或异常链路；地狱还要求架构级判断、多步复杂调试或大量边界场景。文件多、页面多、字段多和题面长不能单独证明难度或代码量。
...
TASK_RUNTIME_WORDING_RULES = """公开题面不要出现“保留现有 Docker Compose 启动方式”，也不要换成“沿用现有启动方式”“保持原有容器启动链路”等同义套话。直接从本题的业务需求或实际故障进入；不要为每道题都加启动说明开头或交付清单结尾。Docker 启动与验收仍是交付约束，但这些内部要求不是要复制到每份题面的句子。0–1 题按实际组件自然说明运行要求；迭代和 Bug 题仅在复现或验收确实需要时写已有的具体启动命令、端口或服务，不编造命令，也不再补一句通用的“保留/沿用”要求。"""
...
def generated_task_prompt_issues(task_type: str, prompt: str,
    ...
    if task_type == "zero_to_one" and candidate is not None:
        infrastructure_patterns = (
            r"Dockerfile", r"Docker\s*Compose|Compose", r"健康检查|healthcheck",
            r"(?:宿主机)?端口", r"\bverify\b", r"退出码|自行退出|自动退出|执行后退出|运行后退出",
        )
        for sentence in sentences:
            ...
            if bundled >= 4:
                issues.append("0–1 新题不得把 Docker、健康检查、端口和 verify 压成基础设施模板句")
                break
        verify_sentences = [
...
def actual_difficulty_review_prompt(task: str, original_difficulty: str,
    ...
    threshold = (
        "Bug 修复的整体实际难度为中等、困难或地狱即可进入录像和 GSB，简单必须停止该 Pair 并换题。"
        if task_type == "bugfix" else
        "0–1 和 Feature 只有整体实际难度仍为困难或地狱才允许进入录像和 GSB，简单或中等必须停止该 Pair 并换题。"
    )
...
还要检查：必须能用 Docker Compose 清洁启动；应包含可操作的功能验收；不能只是改名换皮；与已有题目不能在核心功能、交互、数据模型、验收或技术实现上高度重复。新生成的 0–1 题保留 Dockerfile、Docker Compose、健康检查、可配置宿主机端口和一次性 verify 服务，verify 执行后以退出码报告结果，范围只包括代码测试、构建检查和 API/HTTP 冒烟，不要求项目仓库安装 Playwright 或运行浏览器验收。上述运行要求要结合项目形态分散到相关业务句中；若把 Docker、健康检查、端口和 verify 压成通用结尾或基础设施清单句，应拒绝并重写。去重按业务目标、核心机制和验收链路判断，不能只看标题或字面是否完全一致；相同机制换行业名、换角色名或改写措辞仍算重复。
...
题面从空仓库起步，并保留 Dockerfile、Docker Compose、健康检查、可配置宿主机端口，以及 Compose 中名为 verify、执行验收后自行退出并以退出码报告结果的一次性服务。把这些要求按当前项目的实际运行关系分散表达：交付方式放在组件启动语境，健康检查和端口放在对应服务语境，verify 放在具体自动化验收场景附近；任何一句都不能同时堆叠其中四项或更多，也不能用通用基础设施句收尾。verify 的范围只包括代码测试、构建检查和 API/HTTP 冒烟，不要求项目仓库安装 Playwright 或运行浏览器验收；页面真实操作与录像由交付流程另行完成，不写成项目内 verify 的义务。不要在结尾堆 README、测试、.gitignore 等通用清单。只固定会改变核心验收结果的业务规则，字段命名、页面布局和内部实现留给开发者。内部范围字段只供系统校验，必须如实填写，不能写进 prompt。
...
优先生成实际修复难度达到困难或地狱、合理修复至少涉及 20 行有效生产代码的候选；实际难度为中等时，合理修复必须至少涉及 50 行有效生产代码。不设最低修复时间，但熟悉项目的开发者应能在 120 分钟内完成。改常量、恢复一行条件、补一个显而易见的校验或照现有测试抄答案的缺陷不合格。API 限流、证书、网络临时中断和机器资源不足不是产品 Bug。找不到可信候选时返回空 candidates，禁止编造。
...
题面必须忠实使用候选中的前置条件、操作、真实结果、正确行为和复现结果。不得把预计结果改写成已发生事实，也不得补造接口、文件、数字、原因或测试结论。需要把触发条件、关键操作、用户可见后果和修复后的可执行验收讲清楚。Docker Compose 兼容性由原项目基线和验收链路核对，不要求在题面复述一条固定的启动兼容口号。

{TASK_RUNTIME_WORDING_RULES}

开发者只能看到业务现象和可复现证据。题面不得透露根因，不得写内部文件名、函数名、代码行号、修改位置、补丁策略、建议算法或实现答案；候选中的 sourcePaths 和内部分析只供你核对，不能出现在 prompt。公开接口、页面控件、输入数据、状态码和通过真实业务入口执行的命令可以保留。若现有复现命令直接导入内部函数或读取源码，应把它改写成业务场景说明，并要求修复后通过代码测试、构建检查、HTTP API 或仓库已有的公开验收入口验证，不能把内部调用链发给开发者。

根据这个 Bug 自身的因果关系自然组织文字。不要套用固定开头、固定段落顺序或固定结尾；不要使用“前置条件：”“复现步骤：”“实际结果：”“预期结果：”四段结构，也不要写“请修复该问题，保留现有 Docker Compose 启动与验收链路，并补充覆盖复现路径的自动化验收”。可以使用自然段；只有确实有助于执行时才使用短列表。回归验收要说明真实需要执行的场景和结果，不能只写“补充测试”。题面中的自动化验收从代码测试、构建检查和 API/HTTP 冒烟中选择适合当前项目的方式。最终 prompt 不得出现 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E、真实点击脚本等相关要求，也不要写禁止或无需使用这些工具的说明。
...
只在以下条件全部满足时 accepted=true：候选已经由两次清洁 Docker 环境稳定复现，题面中的事实与复现证据一致；实际修复为困难或地狱且合理修复预计至少涉及 20 行有效生产代码，或者实际修复为中等且合理修复预计至少涉及 50 行有效生产代码；不设最低修复时间，但熟悉项目的开发者预计不得超过 120 分钟；题面能通过真实业务入口复现和验收，同时没有透露根因、内部文件名、函数名、代码行号、修改位置、补丁策略、建议算法或实现答案。

公开 API 路径、页面控件、输入数据、状态码和业务错误可以作为复现锚点，不算答案泄露。直接告诉开发者哪个内部判断错了、应改哪个函数、采用线程池、缓存或动态规划等具体修法，或把只读源码和直接导入内部求解器当成主要验收，均视为 answerLeak=true。公开题面不得出现 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E 或真实点击脚本等相关要求或禁用说明；自动化验收应使用代码测试、构建检查和 API/HTTP 冒烟。不要因为题面长、数字多或业务名复杂就判为困难。

estimatedChangedLines 和 estimatedChangedFiles 评估正确修复的合理生产代码改动量。困难/地狱不足 20 行、中等不足 50 行或预计修复超过 120 分钟都必须拒绝，不能按题面自报难度通过。
...
def seeded_bug_prompt(original_task: str, project_summary: str,
    ...
    return f"""你要为一个隔离的 A/B 修复基准制作带缺陷的初始仓库。直接检查并修改当前目录里的生产代码，只引入一个真实、稳定、可由现有业务入口触发的 Bug；不设最低修复时间，但熟悉项目的开发者应能在 120 分钟内完成。优先制作困难或地狱且合理修复至少需要 20 行有效生产代码的缺陷；实际为中等时，合理修复必须至少需要 50 行有效生产代码。缺陷需要理解跨模块状态、持久化、并发、异常恢复、精确算法或前后端契约中的至少一项关键关系。

只改实现该缺陷所必需的生产代码。注入变更本身必须包含至少 12 行有意义的生产逻辑改动，并形成需要理解状态、算法、并发、持久化、异常恢复或跨层契约才能修复的缺陷；不得通过注释、格式化、重复代码或无关重写凑行数。选定候选、动手修改之前，先反推开发者面对缺陷仓库时最小且正确的通用修法；困难/地狱候选的修法必须至少需要 20 行有效生产逻辑，中等候选必须至少需要 50 行，否则必须换候选。优先破坏一个分布在两个以上状态或模块中的既有不变量，让正确修复必须重新贯通状态模型、持久化或接口契约。若通过删除或回退本次注入、加一把锁、恢复排序、补一个字段或条件、替换公式或常量、修正单个坐标映射等少量改动就能完整修好，即使注入代码很多也不合格。不要新增或修改测试、README、注释、Dockerfile、Compose、verify 脚本、锁文件和依赖清单，也不要留下 TODO、故障开关、补丁说明、答案提示或故意报错文本。不得硬编码只针对一个示例的分支。修改后 Compose 必须能清洁构建和启动，应用服务、健康检查及容器配置仍须正常；来源原有 verify 时，可以因同一业务缺陷返回非零，但不能出现与题面故障无关的构建、依赖、启动或环境失败。旧来源原本没有 verify 时保持原样，不要为了造 Bug 新增验收服务。

动手前先反向设计正确修复的最小方案：如果熟悉仓库的人能够通过删除本次新增分支、恢复输入顺序、携带一个原始索引、补全一个集合或键、改一处比较，或用低于对应代码量门槛的改动解决，就放弃该想法并另选缺陷。除非需要实质重写复杂核心算法，否则正确修复应跨至少两个生产模块或文件，并同时恢复两个以上可观察业务契约。不要为了证明注入成功而安装依赖、下载包或运行 Docker；系统会在你返回后独立执行清洁 Compose 预检。只运行仓库现成且无需安装的快速检查。

同时生成自然中文修复题面。题面说明真实触发条件、操作、实际现象、正确行为和回归验收，但不能透露根因、修改位置、实现方案或你改过哪些文件；不要套用“前置条件/复现步骤/实际结果/预期结果”四段模板。自然写明“自动化验收”的具体场景和应核对结果；自动化部分从代码测试、构建检查和 API/HTTP 冒烟中选择适合当前项目的方式。最终题面不要出现 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E、真实点击脚本等相关要求，也不要写禁止或无需使用这些工具的说明。最终题面必须同时出现“自动化”和“验收”或“测试”。不要加入仓库内已有测试可以直接抄出的答案。
...
完成代码修改后再按 Schema 返回。changedPaths 只列实际改动的生产代码路径；difficultyEvidence 说明真实难度及为什么修复达到对应代码量门槛；stack 只写主要语言与应用框架。"""
...
只在同时满足以下条件时 accepted=true：Bug 可由题面描述的真实业务入口稳定触发；困难或地狱修复预计至少涉及 20 行有效生产代码，或者中等修复预计至少涉及 50 行有效生产代码；不设最低修复时间，但预计不得超过 120 分钟；修复需要理解关键状态、算法、并发、持久化、异常恢复或跨层契约，可以只改一个核心文件，但不能只是改常量、恢复一行条件或照题面抄答案；注入 diff 至少要有 12 行有意义的生产逻辑变更，不能是单行公式、坐标映射、常量或条件替换，也不能靠无关重写凑行数；题面没有出现内部文件名、函数名、修改位置、根因、补丁策略或其他解法提示；题面没有出现 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E、真实点击脚本等相关要求或禁用说明，自动化验收使用代码测试、构建检查和 API/HTTP 冒烟；注入本身不是显眼的故障开关、特例硬编码、故意抛错或破坏启动。
...
def seeded_bug_prompt_rewrite_prompt(title: str, task_prompt: str,
                                     review_reason: str,
                                     existing_tasks: str) -> str:
    return f"""一份隐藏 Bug 修复题已经通过复现、难度和代码量检查，但独立复核认为公开标题或题面泄露了根因、内部约束或补丁方向。只重写标题和题面，不修改代码，也不改变故障事实、输入、接口、可见现象、正确行为和验收范围。

标题和题面要让开发者能从真实业务入口稳定复现问题，但不能直接说内部哪种状态、剪枝、缓存、锁、排序、索引、公式或数据结构写错，也不能提示应删除、恢复、替换或采用哪种实现。公开 API、页面操作、输入数据、状态码和可观察输出可以保留。使用自然中文组织，不套“前置条件/复现步骤/实际结果/预期结果”固定四段，不出现内部文件名、函数名、代码行号和浏览器自动化工具。保留“自动化”以及“验收”或“测试”的要求。

不得为了隐藏答案而删掉关键复现证据，也不得新增原题没有的业务事实。新标题不能概括内部根因，只描述用户看到的错误结果或触发场景；新题面不能与已有 Bug 题换皮重复。
