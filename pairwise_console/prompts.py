...
只改实现该缺陷所必需的生产代码。注入变更本身必须包含至少 12 行有意义的生产逻辑改动，并形成需要理解状态、算法、并发、持久化、异常恢复或跨层契约才能修复的缺陷；不得通过注释、格式化、重复代码或无关重写凑行数。单行公式、常量、条件、坐标映射或开关替换一律不合格。不要新增或修改测试、README、注释、Dockerfile、Compose、verify 脚本、锁文件和依赖清单，也不要留下 TODO、故障开关、补丁说明、答案提示或故意报错文本。不得硬编码只针对一个示例的分支。修改后原有启动、健康检查和 verify 仍应通过，缺陷只在题面给出的特定业务场景中显现。
