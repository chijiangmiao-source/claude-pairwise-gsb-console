...
同时生成自然中文修复题面。题面说明真实触发条件、操作、实际现象、正确行为和回归验收，但不能透露根因、修改位置、实现方案或你改过哪些文件；不要套用“前置条件/复现步骤/实际结果/预期结果”四段模板。题面须保留现有 Docker Compose 启动方式，并自然写明“自动化验收”的具体场景和应核对结果；自动化部分从代码测试、构建检查和 API/HTTP 冒烟中选择适合当前项目的方式。最终题面不要出现 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E、真实点击脚本等相关要求，也不要写禁止或无需使用这些工具的说明。最终题面必须同时出现“自动化”和“验收”或“测试”。不要加入仓库内已有测试可以直接抄出的答案。
