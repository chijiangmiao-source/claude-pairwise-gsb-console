...
写得像给同事讲清楚这次开发：先说具体做了什么，再说真实测试、报错或 Bug，最后自然说明为什么更偏向哪一边。优先使用 traceEvidence 中真实存在的步骤号、命令、文件和结果，例如“第 52 步补了接口测试，第 57 步因 internal 包限制没跑起来，第 58 步换到临时目录后跑通”；不要写成“测试覆盖充分、执行能力较强”这类抽象评语。某次测试先失败、后来修好时，把失败原因和修复后的结果连起来写；不要把已经修好的中间问题说成最终缺陷。若 discoveredBugs 中有已复现问题，要用普通中文说明触发方式、实际结果和是否仍未解决，并让它真实影响比较结论；只有静态猜测或未复现候选时不能当成事实。某一侧没有跑业务测试，也可以直接写“轨迹里没有跑接口业务测试”，不能把编译通过写成业务正确性已验证。

每段至少写出一个真实、可核对的触发节点或证据位置，例如 traceEvidence 给出的第几步、文件名与函数名、实际命令，或报错原文。步骤号只能引用 traceEvidence 的 step，不能根据总行数或调用次数猜测；没有步骤号就改用真实文件、函数、命令或报错定位。只写“后续验收通过”“存在问题”“经过重试”不合格。具体问题还要说明该节点的实际行为和已发生的结果，证据没有提供的细节不得编造。不要因为一侧步骤更多或更少就直接判优劣，重点比较最终功能、真实测试覆盖、未解决 Bug 和恢复过程。使用自然、口语化中文，不使用反引号、Markdown 列表、JSON 文本、模型名称或空泛套话。A、B 各 20–300 个字符。选择 Same 时也要分别说明两边的实际表现和结果接近的依据。
...
重点检查结论与 A/B 事实是否一致，是否把后续独立验收写成开发过程事实，是否遗漏会改变结论的重试、追加提示、已复现 Bug 或未验证范围，是否包含不存在的测试数字、文件、命令或结果。A、B 理由必须分别描述对应产物，并把支持最终偏好的比较依据自然融入两段；不得新增单独的偏好依据。优先把 traceEvidence 里的真实步骤、命令和测试结果改写成通俗叙述：说明做了什么、哪里失败、后来怎么处理以及客观结果。若 discoveredBugs 有已复现问题，评价必须说明触发场景和实际影响；未复现候选不能写成确定缺陷。不能只用“覆盖充分、质量较高、实现完整”等抽象判断，也不能仅凭步骤多少判断优劣。

每段必须至少包含一个真实、可核对的触发节点或证据位置：traceEvidence 中的真实步骤号、文件名与函数名、实际命令、报错原文中的任意一种。步骤号必须与 traceEvidence 的 step 一致。出现具体问题时，还要写清该节点的实际行为和已发生结果；中间失败后来已修好时要同时写出恢复结果，不能冒充最终缺陷。任一段缺少这种定位时不能返回 passed，应返回 suggested_revision，并在建议文本中补齐；证据不足时明确指出缺项，禁止编造。每段使用口语化自然中文，不含反引号、Markdown、JSON 或模型名称。
