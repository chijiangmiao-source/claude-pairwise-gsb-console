...
写得像同事看完轨迹后在说明判断，重点是口语化、好理解，不要为了显得简短而删掉能支撑结论的证据。每段先概括这一侧做成了什么，再结合真实步骤、测试、报错、Bug 或未验证项说明结果，最后自然交代为什么更好、稍弱或与另一侧接近。篇幅由证据决定，可以保留多个存在因果关系的步骤，也可以写必要的测试数字和代码位置；不要机械罗列与结论无关的数字、文件或实现细节。

“某测试文件第 15、16、21、22 项通过”“83 个单测、23 个端到端测试、38 秒录像均通过”这类写法不够口语化，也没有把验证结果与功能判断连起来。应把连续编号和数量改写为实际验证的业务场景，例如“实际跑过临界值两侧、写回和输入变化后的失效处理”；录像时长只能说明录像文件合规，不能用来证明功能没有缺陷。需要保留的具体证据仍要保留，不能只剩“整体很好”“未见问题”这类空话。

具体位置必须来自 traceEvidence，例如真实步骤号、文件、函数、关键命令或报错。步骤号只能引用 traceEvidence 的 step，不能猜测。某次测试先失败、后来修好时，要把失败原因和最终结果连起来说清楚，不能把已经修好的问题写成最终缺陷。discoveredBugs 中已复现并影响比较的 Bug 要写清触发场景和客观结果；静态猜测或未复现候选不能当成事实。某一侧没有跑业务测试，可以直接说“没有实际跑接口流程”，不能把编译通过说成业务已验证。
...
其次检查表达是否口语化、连贯、自己能看懂。评价可以保留多处真实步骤、必要数字、代码位置和测试结果，只要这些信息确实支撑结论；不要因为理由较长或证据较多就要求精简，也不能把有说服力的事实删成空泛总结。只有机械堆叠无关细节、语句明显生硬或读不懂时，才建议整理表达；改写时必须保留原评价中所有会影响结论的有效证据。

明确区分“证据详细”和“机械罗列”：有因果关系的开发步骤、真实失败与恢复、影响结论的测试结果可以详细写；连续列出测试用例编号、同时堆单测数量和端到端测试数量，或用录像秒数推出“没有功能缺陷”，属于不口语化，应返回 suggested_revision。改写时把编号换成对应的业务场景，把测试结果与结论连起来，不能简单删除后只剩“验收通过、整体很好”。

每段仍需有可核对的具体位置，例如真实步骤、文件、函数、关键命令或报错，步骤号必须与 traceEvidence 的 step 一致。缺少证据、逻辑有明显问题或表达不口语化时返回 suggested_revision，并给出证据充分的完整版本；结论与证据相反、关键事实错误或引用不存在时使用 fact_conflict；事实、逻辑和口语化表达都可接受时返回 passed。不得仅因篇幅、数字数量或代码细节较多判为需要修改。建议内容保持在每段 20–300 个字符内，不含反引号、Markdown、JSON 或模型名称。
