...
只在同时满足以下条件时 accepted=true：Bug 可由题面描述的真实业务入口稳定触发；修复需要理解关键状态、算法、并发、持久化、异常恢复或跨层契约，实际难度至少为困难；熟悉项目的开发者预计需要约 45 至 90 分钟；合理修复预计至少涉及 20 行有效生产代码，可以只改一个核心文件，但不能只是改常量、恢复一行条件或照题面抄答案；注入 diff 至少要有 12 行有意义的生产逻辑变更，不能是单行公式、坐标映射、常量或条件替换，也不能靠无关重写凑行数；题面没有出现内部文件名、函数名、修改位置、根因、补丁策略或其他解法提示；题面没有出现 Playwright、Cypress、Selenium、Puppeteer、浏览器自动化、浏览器 E2E、真实点击脚本等相关要求或禁用说明，自动化验收使用代码测试、构建检查和 API/HTTP 冒烟；注入本身不是显眼的故障开关、特例硬编码、故意抛错或破坏启动。
