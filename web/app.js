...
async function renderReviews() {
  ...
  const rows = data.items.map((row) => `<article class="review-list-row">
    ...
    <div class="review-summary"><strong>${esc(row.title)}</strong><small>项目编号：${esc(row.project_number)}</small>${pairIdentity(row.pair_id)}<div class="tag-stack">${taskTypeBadge(row.task_type)} ${projectCategoryBadge(row.project_category)} <span class="difficulty">${esc(row.difficulty)}</span></div><small>${date(row.confirmed_at || row.updated_at)}</small></div>
