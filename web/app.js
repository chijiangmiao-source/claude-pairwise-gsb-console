...
async function renderEvidence() {
  ...
  $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>Docker 产物验收与真实操作录像</h2><p class="sub">每行对应一个 Pair 的一个 Arm，可直接播放最终提交的真实演示</p></div></div>
    ...
    ${table(data.items, [["项目 / Arm", (row) => `<div class="title-cell"><strong>${esc(row.title)} · ${row.arm}</strong><small>${esc(row.project_number)} · ${esc(row.pair_id)}</small></div>`], ["类型 / 难度", (row) => `${esc(row.task_type)} · <span class="difficulty">${esc(row.difficulty)}</span>`], ["最终提交", (row) => `<span class="code">${esc((row.commit_sha || "").slice(0, 12))}</span>`], ["Docker 验收", (row) => badge(row.artifact_status || "missing")], ["真实操作录像", (row) => `${badge(row.latest_attempt_status === "starting" ? "starting" : row.latest_attempt_status === "recording" ? "recording" : row.recording_status || "missing")}<small class="block">${row.capture_mode === "browser" ? "浏览器视口" : row.recording_id ? "历史屏幕录像" : ""} ${row.width || 0}×${row.height || 0} · ${row.duration_seconds || 0}s</small>`], ["提交匹配", (row) => row.commit_match ? '<span class="ok">● 匹配</span>' : '<span class="bad">● 未匹配</span>'], ["操作", evidenceActions]])}${pager("evidence", data)}</div>`;
}
