...
const pages = {
  ...
  tasks: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>题目池</h2><p class="sub">0–1/Feature 需困难或地狱；Bug 修复最低可为中等</p></div><div class="toolbar-group"><button class="secondary" onclick="importTasks()">导入历史困难题</button>${automationAction}</div></div>
      <div class="automation-strip ${automation.enabled ? "enabled" : ""}"><div><strong>${automation.enabled ? "完整流程持续运行" : "完整流程尚未启动"}</strong><small>${automation.enabled ? "现有合格题按入库顺序使用；空位自动补 Pair，题目不足时从真实可用来源补题" : "点击后自动完成仓库准备、A/B 开发、Docker 验收、录像和 GSB"}</small></div><div class="automation-metrics"><span>活动 Pair <b>${automation.activePairs}/${automation.targetPairs}</b></span>${automation.waitingApiArms ? `<span>等待 API 重试 <b>${automation.waitingApiArms}</b></span>` : ""}<span>可用题目 <b>${automation.readyTasks}</b></span><span>选题方式 <b>有题即用</b></span><span>${esc(stageText)}</span></div></div>
