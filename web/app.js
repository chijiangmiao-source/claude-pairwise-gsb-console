...
const pages = {
  dashboard: async () => {
    ...
    $("#content").innerHTML = `<div class="grid stats">
      ...
      <div class="stat"><small>已确认 GSB</small><strong>${summary.confirmedGsb}</strong><em>A better / Same / B better</em></div>
      <div class="stat"><small>24 小时活跃时段</small><strong>${summary.activeHours24h}</strong><em>平均每小时 ${summary.hourlyAverage24h}</em></div></div>
      ...
      <div class="grid two-col" style="margin-top:16px"><div class="card"><h2>Pair 状态</h2><p class="sub">当前状态机中的全部项目</p>${distribution(data.pairStatus)}</div><div class="card"><h2>交付证据</h2><p class="sub">Docker 验收与录像状态</p>${distribution([...data.artifactChecks, ...data.recordings])}</div></div>`;
  },
  tasks: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>题目池</h2><p class="sub">困难与地狱题目进入 A/B</p></div><div class="toolbar-group"><button class="secondary" onclick="importTasks()">导入历史困难题</button><button class="primary" onclick="generateTask()">自动补题</button></div></div>${table(data.items, [
      ...
      ["类型", (row) => esc(row.task_type)], ["难度", (row) => `<span class="difficulty">${esc(row.difficulty)}</span>`],
  ...
  pairs: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>A/B 项目</h2><p class="sub">同一 main 基线，大写 A/B 分支，两个独立 Claude 容器</p></div></div>${table(data.items, [
      ...
      ["类型 / 难度", (row) => `${esc(row.task_type)} · <span class="difficulty">${esc(row.difficulty)}</span>`],
...
async function renderEvidence() {
  ...
  $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>Docker 产物验收与真实操作录像</h2><p class="sub">每行对应一个 Pair 的一个 Arm，可直接播放最终提交的真实演示</p></div></div>
    ${filterBar("evidence", [["q", "search", "项目编号、Pair、题目或提交", filters.q], ["arm", "select", "全部 Arm", filters.arm, ["A", "B"]], ["task_type", "select", "全部任务类型", filters.task_type, ["zero_to_one", "feature", "bugfix"]], ["artifact_status", "select", "全部验收状态", filters.artifact_status, ["queued", "running", "passed", "failed"]], ["recording_status", "select", "全部录像状态", filters.recording_status, ["queued", "recording", "passed", "failed"]], ["missing", "select", "全部资料", filters.missing, [["1", "只看缺失或失败"]]]])}
    ${table(data.items, [["项目 / Arm", (row) => `<div class="title-cell"><strong>${esc(row.title)} · ${row.arm}</strong><small>${esc(row.project_number)} · ${esc(row.pair_id)}</small></div>`], ["类型 / 难度", (row) => `${esc(row.task_type)} · <span class="difficulty">${esc(row.difficulty)}</span>`], ["最终提交", (row) => `<span class="code">${esc((row.commit_sha || "").slice(0, 12))}</span>`], ["Docker 验收", (row) => badge(row.artifact_status || "missing")], ["真实操作录像", (row) => `${badge(row.latest_attempt_status === "starting" ? "starting" : row.latest_attempt_status === "recording" ? "recording" : row.recording_status || "missing")}<small class="block">${[recordingFormat(row), row.capture_mode === "browser" ? "浏览器视口" : row.recording_id ? "历史屏幕录像" : ""].filter(Boolean).join(" · ")} ${row.width || 0}×${row.height || 0} · ${row.duration_seconds || 0}s</small>`], ["提交匹配", (row) => row.commit_match ? '<span class="ok">● 匹配</span>' : '<span class="bad">● 未匹配</span>'], ["操作", evidenceActions]])}${pager("evidence", data)}</div>`;
}
...
function evidenceActions(row) {
  ...
  } else if (row.artifact_status !== "passed" || !row.commit_sha) {
    record = '<button class="tiny" disabled>等待 Docker 验收</button>';
  } else {
    record = `<button class="tiny primary" onclick="recordAction('${row.pair_id}','${row.arm}','start')">${row.recording_id ? "不合格并重新录制" : "一键启动并录制"}</button>`;
  }
...
async function renderExports() {
  ...
  $("#content").innerHTML = `<div class="card"><div class="toolbar export-heading"><div><h2>完成 Pair 导出与正式提交</h2><p class="sub">0–1、Feature 和 Bug Pair 分别占一行；资料不完整时仍可导出复核副本</p></div><div class="toolbar-group"><b>已选 ${state.exportSelection.size} 项</b><button class="secondary" onclick="preflightSelected()" ${state.exportSelection.size ? "" : "disabled"}>提交前检查</button><button class="secondary" onclick="selectReady()">只选可提交项</button><button class="secondary" onclick="exportSelected()" ${state.exportSelection.size ? "" : "disabled"}>导出 Excel</button><button class="primary" onclick="submitSelected()" ${state.exportSelection.size ? "" : "disabled"}>正式提交</button><button class="danger" onclick="hideSelected()" ${state.exportSelection.size ? "" : "disabled"}>从列表隐藏</button></div></div>
    ${filterBar("exports", [["q", "search", "项目编号、Pair、题目或题面", filters.q], ["task_type", "select", "全部任务类型", filters.task_type, ["zero_to_one", "feature", "bugfix"]], ["difficulty", "select", "全部难度", filters.difficulty, ["困难", "地狱"]], ["readiness", "select", "全部资料状态", filters.readiness, [["ready", "可提交"], ["blocked", "待补资料"]]], ["submission_status", "select", "全部提交状态", filters.submission_status, [["ready_to_submit", "待提交"], ["submitted", "已提交"], ["needs_review", "待重新确认"]]], ["date_from", "date", "完成日期从", filters.date_from], ["date_to", "date", "完成日期到", filters.date_to], ["include_hidden", "select", "未隐藏记录", filters.include_hidden, [["1", "包含已隐藏"]]]])}
...
function distribution(rows) { if (!rows.length) return '<div class="empty">还没有数据</div>'; const max = Math.max(...rows.map((item) => item.count)); return `<div class="legend-list">${rows.map((item) => `<div class="legend-row"><span>${esc(item.verdict || item.status || item.task_type)}</span><div class="legend-track"><div class="legend-fill" style="width:${item.count / max * 100}%"></div></div><b>${item.count}</b></div>`).join("")}</div>`; }
...
async function recordAction(id, arm, action) { try { if ($("#dialog")?.open) $("#dialog").close(); await api(`/api/pairs/${id}/recordings/${arm}/${action}`, { method: "POST", body: JSON.stringify({}) }); notify(action === "start" ? `${arm} 正在启动 Docker 项目；独立浏览器打开后会自动开始页面录像` : `${arm} 录像正在保存并做技术检查`); if (state.page === "evidence") renderEvidence(); watchRecording(id, arm); } catch (error) { notify(error.message, true); } }
