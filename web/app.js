...
const pages = {
  dashboard: async () => {
    ...
    $("#content").innerHTML = `<div class="grid stats">
      ...
      <div class="stat"><small>累计完成 Pair</small><strong>${summary.completedPairs}</strong><em>人工确认后的正式记录</em></div>
      ...
      <div class="grid two-col"><div class="card"><h2>24 小时产出趋势</h2><p class="sub">以上海日期归档，统计完成的 A/B Pair</p><div class="bar-chart">${Array.from({ length: 24 }, (_, index) => { const hour = String(index).padStart(2, "0"); const count = data.trend24h.find((item) => item.hour?.endsWith(hour))?.count || 0; return `<div class="bar-wrap" title="${hour}:00 · ${count} 条"><div class="bar" style="height:${Math.max(2, count / max * 135)}px"></div><small>${index % 3 === 0 ? hour : ""}</small></div>`; }).join("")}</div></div>
      <div class="card"><h2>GSB 分布</h2><p class="sub">只统计人工确认记录</p>${distribution(data.verdicts)}</div></div>
  ...
  codex: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><h2>Codex CLI 作业</h2><p class="sub">除 A/B 开发外的模型节点均在这里留痕</p>${table(data.items, [
      ["作业", (row) => `<div class="title-cell"><strong>${esc(row.job_type)}</strong><small>${esc(row.id)}</small></div>`],
  ...
  bugs: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><h2>真实 Bug 复现</h2><p class="sub">困难或地狱候选必须在两次清洁 Compose 环境得到一致结果</p>${table(data.items, [
      ...
      ["来源", (row) => `${esc(row.source_pair_id)} · ${row.source_arm}`], ["难度", (row) => `<span class="difficulty">${esc(row.difficulty)}</span>`],
...
function evidenceActions(row) {
  const play = row.recording_id && row.recording_status === "passed"
    ? `<button class="tiny" onclick="playRecording('${row.recording_id}','${row.arm}','${esc(row.title)}')">播放</button>` : "";
  let record;
...
async function renderReviews() {
  ...
  const rows = data.items.map((row) => `<article class="review-list-row">
    ...
    <div class="review-reason"><span class="mobile-label">公开理由</span><p><b>A：</b>${esc(row.a_reason || "未填写")}</p><p><b>B：</b>${esc(row.b_reason || "未填写")}</p><p><b>偏好依据：</b>${esc(row.preference_reason || row.reason || "未填写")}</p></div>
  ...
  $("#content").innerHTML = `<div class="card review-page"><div class="toolbar review-heading"><div><h2>复核与人工确认</h2><p class="sub">公开理由可用高强度模型复检；事实冲突会阻止正式导出，措辞建议不会阻止确认</p></div><div class="review-batch-actions"><b>已选 ${state.reviewSelection.size} 项</b><label><input type="checkbox" ${allSelected ? "checked" : ""} onchange="toggleReviewPage(this.checked)"> 选择本页</label><button class="primary" onclick="recheckReviewsSelected()" ${state.reviewSelection.size ? "" : "disabled"}>批量复检</button></div></div>
    ${filterBar("reviews", [["q", "search", "项目编号、Pair、题目、理由或审核人", filters.q], ["task_type", "select", "全部任务类型", filters.task_type, [["zero_to_one", "0–1"], ["feature", "Feature 迭代"], ["bugfix", "Bug 修复"]]], ["project_category", "select", "全部系统类型", filters.project_category, ["纯后端", "纯前端", "全栈"]], ["difficulty", "select", "全部难度", filters.difficulty, ["困难", "地狱"]], ["verdict", "select", "全部结论", filters.verdict, ["A better", "Same", "B better"]], ["status", "select", "全部确认状态", filters.status, [["draft", "草稿"], ["confirmed", "已确认"]]], ["recheck_status", "select", "全部复检状态", filters.recheck_status, [["passed", "复检通过"], ["suggested_revision", "建议修改"], ["fact_conflict", "事实冲突"]]], ["date_from", "date", "记录日期从", filters.date_from], ["date_to", "date", "记录日期到", filters.date_to]])}
...
function distribution(rows) { if (!rows.length) return '<div class="empty">还没有数据</div>'; const max = Math.max(...rows.map((item) => item.count)); return `<div class="legend-list">${rows.map((item) => { const raw = item.project_category || item.task_type || item.verdict || item.status; const label = item.task_type ? (taskTypeLabels[item.task_type] || item.task_type) : raw; return `<div class="legend-row"><span>${esc(label)}</span><div class="legend-track"><div class="legend-fill ${item.project_category ? categoryTone(item.project_category) : item.task_type || ""}" style="width:${item.count / max * 100}%"></div></div><b>${item.count}</b></div>`; }).join("")}</div>`; }
function pairOverviewCard(row) { return `<button class="pair-overview-card ${categoryTone(row.project_category)}" onclick="showPair('${row.pair_id}')"><span class="pair-overview-head"><strong>${esc(row.title)}</strong>${badge(row.status)}</span><small>${esc(row.project_number)} · ${esc(row.pair_id)}</small><span class="tag-stack">${projectCategoryBadge(row.project_category)} ${taskTypeBadge(row.task_type)} <span class="difficulty">${esc(row.difficulty)}</span></span><span class="pair-progress"><span>Docker ${Number(row.checks_passed || 0)}/2</span><span>录像 ${Number(row.recordings_passed || 0)}/2</span><span>${row.verdict ? esc(row.verdict) : esc(row.stage)}</span></span><small>更新于 ${date(row.completed_at || row.updated_at)}</small></button>`; }
...
async function showPair(id) { try { const data = await api(`/api/pairs/${id}`), repo = data.repository; showDialog(`<h2>${esc(data.task.title)}</h2><p>${projectCategoryBadge(data.task.project_category)} ${taskTypeBadge(data.task.task_type)} ${badge(data.status)} ${badge(data.stage)}</p><div class="detail-grid"><div class="detail-item"><label>Pair ID</label><strong class="code">${esc(data.id)}</strong></div><div class="detail-item"><label>共同基线</label><strong class="code">${esc(data.baseline_sha || "待创建")}</strong></div><div class="detail-item"><label>远端仓库</label><strong class="code">${esc(repo?.remote_url || "待创建")}</strong></div><div class="detail-item"><label>分支</label><strong>main / A / B</strong></div></div><h3>A/B 执行</h3>${table(data.arms, [["Arm", (row) => row.arm], ["SessionID", (row) => `<span class="code">${esc(row.session_id)}</span>`], ["PromptID", (row) => `<span class="code">${esc(row.prompt_id)}</span>`], ["状态", (row) => badge(row.status)], ["提交", (row) => `<span class="code">${esc(row.commit_sha)}</span>`]])}<div style="margin-top:18px">${pairButtons(data)}</div>`); } catch (error) { notify(error.message, true); } }
function pairButtons(data) { const id = data.id; if (data.stage === "repository") return `<button class="primary" onclick="pairAction('${id}','prepare')">创建 GitHub 仓库并准备 A/B</button>`; if (data.stage === "ready_to_start") return `<button class="primary" onclick="pairAction('${id}','start')">并行启动 A/B 开发</button>`; if (data.stage === "recording") { const byArm = Object.fromEntries((data.recordings || []).map((item) => [item.arm, item])); return ["A", "B"].map((arm) => byArm[arm]?.status === "recording" ? `<button class="danger" onclick="recordAction('${id}','${arm}','stop')">停止 ${arm} 录像</button>` : byArm[arm]?.status === "passed" ? `<span class="badge passed">${arm} 录像通过</span>` : `<button class="primary" onclick="recordAction('${id}','${arm}','start')">录制 ${arm} 真实操作</button>`).join(" "); } if (data.stage === "gsb_ready") return `<button class="primary" onclick="pairAction('${id}','gsb')">生成 GSB 草稿</button>`; if (data.status === "completed") return `<button class="primary" onclick="pairAction('${id}','bugs/discover')">Codex 找 Bug</button>`; return `<span class="badge ${esc(data.status)}">当前阶段：${esc(data.stage)}</span>`; }
async function recordAction(id, arm, action, manual = false) { try { if ($("#dialog")?.open) $("#dialog").close(); await api(`/api/pairs/${id}/recordings/${arm}/${action}`, { method: "POST", body: JSON.stringify({ manual }) }); notify(action === "start" ? (manual ? `${arm} 项目已启动；请在独立 Chrome 中人工操作真实功能，完成后回本页停止并保存` : `${arm} 正在启动 Docker 项目并自动演示真实功能`) : `${arm} 录像正在保存并做技术检查`); if (state.page === "evidence") renderEvidence(); watchRecording(id, arm); } catch (error) { notify(error.message, true); } }
async function watchRecording(id, arm) { for (let attempt = 0; attempt < 360; attempt += 1) { await new Promise((resolve) => window.setTimeout(resolve, 2000)); try { const pair = await api(`/api/pairs/${id}`), latest = pair.recording_attempts?.find((item) => item.arm === arm); if (state.page === "evidence") await renderEvidence(); if (latest && !["starting", "recording"].includes(latest.status)) { notify(latest.status === "passed" ? `${arm} 浏览器录像已通过并自动替换当前录像` : `${arm} 录像失败：${latest.error}`, latest.status !== "passed"); return; } } catch (error) { notify(error.message, true); return; } } }
...
function playRecording(id, arm, title) { showDialog(`<h2>${esc(title)} · ${esc(arm)} 真实操作录像</h2><video class="recording-player" controls autoplay preload="metadata" src="/api/recordings/${encodeURIComponent(id)}/content"></video><p class="sub">支持播放、暂停、拖动进度和全屏。</p>`); }
async function showEvidence(pairId, arm) { try { const data = await api(`/api/pairs/${pairId}`), check = data.checks.find((item) => item.arm === arm) || {}, recording = data.recordings.find((item) => item.arm === arm) || {}, attempts = (data.recording_attempts || []).filter((item) => item.arm === arm), steps = parseJson(check.checks_json, []); showDialog(`<h2>${esc(data.task.title)} · ${arm}</h2><div class="detail-grid"><div class="detail-item"><label>最终提交</label><strong class="code">${esc(check.commit_sha || recording.commit_sha || "—")}</strong></div><div class="detail-item"><label>Docker 验收</label><strong>${badge(check.status || "missing")}</strong></div><div class="detail-item"><label>当前录像</label><strong>${recordingFormat(recording)} · ${recording.capture_mode === "browser" ? "浏览器视口 · " : ""}${recording.width || 0}×${recording.height || 0} · ${recording.duration_seconds || 0}s</strong></div><div class="detail-item"><label>提交匹配</label><strong>${recording.commit_match ? "匹配" : "未匹配"}</strong></div></div><h3>录像历史</h3>${table(attempts, [["时间", (row) => date(row.created_at)], ["方式", (row) => `${recordingFormat(row)} · ${row.interaction_mode === "manual" ? "人工操作" : "自动操作"} · ${row.capture_mode === "browser" ? "浏览器视口" : "历史屏幕录像"}`], ["状态", (row) => badge(row.status)], ["规格", (row) => `${row.width || 0}×${row.height || 0} · ${row.duration_seconds || 0}s`], ["说明", (row) => esc(row.error || row.entry_url || "—")]])}<h3>验收步骤</h3>${table(steps, [["检查", (row) => esc(row.name)], ["结果", (row) => row.passed ? '<span class="ok">通过</span>' : '<span class="bad">失败</span>'], ["详情", (row) => `<div class="wrap log-detail">${esc(row.detail)}</div>`]])}${recording.id && recording.status === "passed" ? `<button class="primary" onclick="playRecording('${recording.id}','${arm}','${esc(data.task.title)}')">播放当前录像</button>` : ""}`); } catch (error) { notify(error.message, true); } }
...
async function reviewGsb(id) {
  try {
    ...
    showDialog(`<h2>复核与人工确认</h2><p>${esc(data.task.title)}</p><div class="ab-review-grid"><div class="review-side"><b>A</b><small>提交 ${esc((arms.A?.commit_sha || "").slice(0, 12))}</small><small>Session ${esc(arms.A?.session_id || "缺失")}</small>${recs.A?.id ? `<button type="button" class="tiny" onclick="playRecording('${recs.A.id}','A','${esc(data.task.title)}')">播放 A</button>` : ""}</div><div class="review-side"><b>B</b><small>提交 ${esc((arms.B?.commit_sha || "").slice(0, 12))}</small><small>Session ${esc(arms.B?.session_id || "缺失")}</small>${recs.B?.id ? `<button type="button" class="tiny" onclick="playRecording('${recs.B.id}','B','${esc(data.task.title)}')">播放 B</button>` : ""}</div></div><div class="choice-row">${["A better", "Same", "B better"].map((value) => `<label class="choice"><input type="radio" name="verdict" value="${value}" ${gsb.verdict === value ? "checked" : ""}>${value}</label>`).join("")}</div><label class="review-field"><b>A 的评价</b><textarea class="review-input" id="gsb-a-reason" maxlength="300">${esc(gsb.a_reason || legacy)}</textarea></label><label class="review-field"><b>B 的评价</b><textarea class="review-input" id="gsb-b-reason" maxlength="300">${esc(gsb.b_reason || legacy)}</textarea></label><label class="review-field"><b>偏好依据</b><textarea class="review-input compact" id="gsb-preference-reason" maxlength="240">${esc(gsb.preference_reason || legacy)}</textarea></label><p class="sub">A、B 分别填写，偏好依据单独说明；禁止反引号和 Markdown。</p>${latest ? `<div class="recheck-result ${esc(latest.result_status)}"><strong>最近复检：${esc(statusLabels[latest.result_status] || latest.result_status)}</strong><small>${esc(latest.model)} · ${esc(latest.reasoning_effort)} · ${date(latest.created_at)}</small>${issues.map((issue) => `<p>${esc(issue)}</p>`).join("")}<div class="review-text"><p><b>A 建议：</b>${esc(latest.suggested_a_reason || "—")}</p><p><b>B 建议：</b>${esc(latest.suggested_b_reason || "—")}</p><p><b>偏好建议：</b>${esc(latest.suggested_preference_reason || latest.suggested_reason || "—")}</p></div><button type="button" class="secondary" onclick="applyRecheck('${id}','${latest.id}')">应用复检建议</button></div>` : '<div class="recheck-result"><strong>尚未复检</strong><p>可使用高强度模型检查公开理由与证据是否一致。</p></div>'}<div class="review-actions"><button type="button" class="secondary" onclick="recheckGsb('${id}')">复检公开理由</button><input id="confirmed-by" value="${esc(gsb.confirmed_by || "刘昱")}" placeholder="确认人"><button type="button" class="primary" onclick="confirmGsb('${id}')">确认并完成</button></div>`);
  } catch (error) { notify(error.message, true); }
}
async function confirmGsb(id) { const verdict = $('input[name="verdict"]:checked')?.value, aReason = $("#gsb-a-reason").value, bReason = $("#gsb-b-reason").value, preferenceReason = $("#gsb-preference-reason").value, confirmedBy = $("#confirmed-by").value; try { await api(`/api/pairs/${id}/gsb/confirm`, { method: "POST", body: JSON.stringify({ verdict, aReason, bReason, preferenceReason, confirmedBy }) }); $("#dialog").close(); notify("GSB 已人工确认，记录进入待正式提交状态"); render(); } catch (error) { notify(error.message, true); } }
...
async function repairSelected() { let applied = 0; try { for (const id of [...state.exportSelection]) { const pair = await api(`/api/pairs/${id}`), latest = pair.gsb_rechecks?.[0]; if (!latest || latest.result_status === "passed") continue; await api(`/api/pairs/${id}/gsb/recheck/${latest.id}/apply`, { method: "POST", body: "{}" }); applied += 1; } notify(applied ? `已对 ${applied} 条记录应用复检建议，请重新人工确认` : "所选记录没有可应用的复检建议"); render(); } catch (error) { notify(error.message, true); } }
async function showDelivery(id) { try { const data = await api(`/api/pairs/${id}`), repository = data.repository || {}, arms = Object.fromEntries(data.arms.map((item) => [item.arm, item])), recordings = Object.fromEntries(data.recordings.map((item) => [item.arm, item])); const commitLink = (arm) => repository.remote_url && arms[arm]?.commit_sha ? `${repository.remote_url.replace(/\.git$/, "")}/commit/${arms[arm].commit_sha}` : ""; showDialog(`<h2>${esc(data.task.title)}</h2><p>${taskTypeBadge(data.task.task_type)} ${projectCategoryBadge(data.task.project_category)} ${badge(data.task.difficulty)} ${badge(data.delivery?.status || "not_submitted")}</p><div class="kv"><b>项目编号</b><span class="code">${esc(data.chain_id)}</span></div><div class="kv"><b>Pair ID</b><span class="code">${esc(data.id)}</span></div><div class="kv"><b>题面</b><div class="wrap">${esc(data.task.prompt)}</div></div>${["A", "B"].map((arm) => `<h3>${arm} 交付资料</h3><div class="detail-grid"><div class="detail-item"><label>SessionID</label><strong class="code">${esc(arms[arm]?.session_id || "缺失")}</strong></div><div class="detail-item"><label>PromptID</label><strong class="code">${esc(arms[arm]?.prompt_id || "缺失")}</strong></div><div class="detail-item"><label>提交</label><strong class="code">${esc(arms[arm]?.commit_sha || "缺失")}</strong>${commitLink(arm) ? `<a href="${esc(commitLink(arm))}" target="_blank" rel="noopener">永久链接</a>` : ""}</div><div class="detail-item"><label>录像</label><strong>${recordings[arm] ? `${recordings[arm].width}×${recordings[arm].height} · ${recordings[arm].duration_seconds}s` : "缺失"}</strong>${recordings[arm]?.id ? `<button class="tiny" onclick="playRecording('${recordings[arm].id}','${arm}','${esc(data.task.title)}')">播放</button>` : ""}</div></div>`).join("")}<h3>GSB</h3><p><b>${esc(data.gsb?.verdict || "无结论")}</b></p><div class="review-text"><p><b>A：</b>${esc(data.gsb?.a_reason || "未填写")}</p><p><b>B：</b>${esc(data.gsb?.b_reason || "未填写")}</p><p><b>偏好依据：</b>${esc(data.gsb?.preference_reason || data.gsb?.reason || "未填写")}</p></div><p class="sub">审核人：${esc(data.gsb?.confirmed_by || "未确认")} · ${date(data.gsb?.confirmed_at)}</p><button class="secondary" onclick="reviewGsb('${id}')">打开复核与人工确认</button>`); } catch (error) { notify(error.message, true); } }
...
async function saveSettings() { const body = {}; $$('[data-setting]').forEach((input) => { body[input.dataset.setting] = ["task_generation_max_parallel", "task_pool_min_ready", "task_pool_target_ready", "max_pairs_parallel"].includes(input.dataset.setting) ? Number(input.value) : input.value; }); try { await api("/api/settings", { method: "POST", body: JSON.stringify(body) }); notify("设置已保存，新作业将使用这些值"); render(); } catch (error) { notify(error.message, true); } }
