...
const pages = {
  ...
  tasks: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>题目池</h2><p class="sub">困难与地狱题目进入 A/B</p></div><div class="toolbar-group"><button class="secondary" onclick="importTasks()">导入历史困难题</button>${automationAction}</div></div>
      <div class="automation-strip ${automation.enabled ? "enabled" : ""}"><div><strong>${automation.enabled ? "完整流程持续运行" : "完整流程尚未启动"}</strong><small>${automation.enabled ? "现有合格题优先；空位自动补 Pair，题目不足按 0–1／迭代／Bug 配比补题" : "点击后自动完成仓库准备、A/B 开发、Docker 验收、录像和 GSB"}</small></div><div class="automation-metrics"><span>活动 Pair <b>${automation.activePairs}/${automation.targetPairs}</b></span><span>可用题目 <b>${automation.readyTasks}</b></span><span>出题配比 <b>${automation.taskMixTarget?.zero_to_one || 7}:${automation.taskMixTarget?.feature || 7}:${automation.taskMixTarget?.bugfix || 10}</b></span><span>${esc(stageText)}</span></div></div>
