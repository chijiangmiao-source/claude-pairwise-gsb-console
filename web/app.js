...
const statusLabels = {
  ...
  artifact_failed: "Docker 验收失败", observed_failed: "Docker 启动失败（已留证）",
  business_test_failed: "Docker 已启动／业务测试有缺陷", artifact_failed_evaluated: "启动失败，已完成 GSB",
  ...
  manual_queue: "等待手动排队", manual_waiting: "等待手动排队", manual_preparing: "正在准备重跑", paused: "已暂停",
  queued: "排队中", running: "运行中", review: "待复核", developing: "开发中", stopping: "正在保存",
  waiting_terminal_slot: "等待终端名额", waiting_retry: "等待重试", waiting_api_retry: "等待 API 自动重试", checkpointing: "正在收尾", active: "进行中",
...
const artifactDisplayStatus = (row) => {
  if ((row?.status || row?.artifact_status) !== "observed_failed") return row?.status || row?.artifact_status || "missing";
  const checks = parseJson(row?.checks_json, []);
  const failed = Array.isArray(checks) ? checks.filter((item) => item && item.passed === false) : [];
  const runtimeFailure = failed.some((item) => /compose|dockerfile|clean_start|containers?_running|health|startup|published_port/i.test(item.name || ""));
  return failed.length && !runtimeFailure ? "business_test_failed" : "observed_failed";
};
const artifactBadge = (row) => badge(artifactDisplayStatus(row));
...
const difficultyDisplay = (row) => {
  ...
  const edit = difficultyEditButton(row.pair_id || (String(row.id || "").startsWith("pair-") ? row.id : ""));
  if (assessed) return `${edit}<span class="difficulty">出题 ${esc(original)} → 实际 ${esc(assessed)}</span>${row.difficulty_review_status ? `<small class="block">${badge(row.difficulty_review_status)}</small>` : ""}`;
  if (row.difficulty_review_status) return `${edit}<span class="difficulty">出题 ${esc(original)}</span><small class="block">${badge(row.difficulty_review_status)}</small>`;
  return `${edit}<span class="difficulty">${esc(row.difficulty || original)}</span>`;
};
...
const pages = {
  dashboard: async () => {
    ...
    $("#content").innerHTML = `<div class="grid stats">
      ...
      <div class="stat"><small>当前活动 Pair</small><strong>${summary.activePairs}</strong><em>排队、开发与评审；等待 API 的任务另行排队</em></div>
  ...
  tasks: async () => {
    ...
    const taskSelectionText = ({
      zero_to_one_only: "仅 0–1",
      feature_only: "仅 Feature 迭代",
      bugfix_only: "仅 Bug 修复",
      available_first: "有题即用",
    })[automation.taskSelectionMode] || "有题即用";
    $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>题目池</h2><p class="sub">0–1/Feature 需困难或地狱；Bug 修复最低可为中等</p></div><div class="toolbar-group"><button class="secondary" onclick="importTasks()">导入历史困难题</button>${automationAction}</div></div>
      <div class="automation-strip ${automation.enabled ? "enabled" : ""}"><div><strong>${automation.enabled ? "完整流程持续运行" : "完整流程尚未启动"}</strong><small>${automation.enabled ? "现有合格题按入库顺序使用；出现 429 时暂停新终端，冷却后每次只恢复一侧" : "点击后自动完成仓库准备、A/B 开发、Docker 验收、录像和 GSB"}</small></div><div class="automation-metrics"><span>活动 Pair <b>${automation.activePairs}/${automation.targetPairs}</b></span><span>开发终端 <b>${automation.activeTerminals ?? 0}/${automation.targetTerminals ?? 3}</b></span>${automation.waitingTerminalArms ? `<span>等待终端 <b>${automation.waitingTerminalArms}</b></span>` : ""}${automation.waitingApiPairs ? `<span>API 排队 Pair <b>${automation.waitingApiPairs}</b></span>` : ""}${automation.waitingApiArms ? `<span>等待重试侧 <b>${automation.waitingApiArms}</b></span>` : ""}${automation.apiCooldownUntil ? `<span>API 冷却至 <b>${esc(new Date(automation.apiCooldownUntil).toLocaleTimeString("zh-CN", {hour12:false}))}</b></span>` : ""}<span>可用题目 <b>${automation.readyTasks}</b></span><span>选题方式 <b>${taskSelectionText}</b></span><span>${esc(stageText)}</span></div></div>
  ...
  pairs: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>A/B 项目</h2><p class="sub">同一 main 基线，大写 A/B 分支，两个独立 Claude 容器</p></div></div>
      ${filterBar("pairs", [["q", "search", "Pair 唯一 ID、项目编号或题目", filters.q], ["task_type", "select", "全部任务类型", filters.task_type, [["zero_to_one", "0–1"], ["feature", "Feature 迭代"], ["bugfix", "Bug 修复"]]], ["project_category", "select", "全部系统类型", filters.project_category, ["纯后端", "纯前端", "全栈"]], ["difficulty", "select", "全部难度", filters.difficulty, ["简单", "中等", "困难", "地狱"]], ["stage", "select", "全部项目阶段", filters.stage, [["manual_queue", "等待手动排队"], ["repository", "仓库准备"], ["ready_to_start", "等待启动"], ["development", "A/B 开发"], ["artifact_validation", "Docker 验收"], ["difficulty_review", "实际难度复评"], ["recording", "正在录制"], ["gsb_ready", "等待 GSB"], ["gsb_confirmation", "GSB 确认"], ["completed", "已完成"], ["baseline_preflight_failed", "基线预检失败"], ["artifact_failed", "Docker 验收失败"], ["artifact_failed_evaluated", "启动失败，已完成 GSB"], ["difficulty_rejected", "难度复评未通过"], ["development_failed", "开发失败"], ["recording_failed", "录像失败"], ["task_replacement", "正在自动换题"], ["replacement_failed", "自动换题失败"]]], ["status", "select", "全部项目状态", filters.status, [["queued", "排队中"], ["paused", "等待手动排队"], ["cancelled", "已取消"], ["running", "运行中"], ["waiting_api_retry", "等待 API 自动重试"], ["review", "待复核"], ["completed", "已完成"], ["failed", "失败"]]]])}
      ${table(data.items, [
      ...
      ["操作", (row) => `<button class="tiny" onclick="showPair('${row.id}')">题目 / 管理</button>`],
  ...
  settings: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><h2>运行环境预检</h2><p class="sub">Git、Codex、Claude 容器和浏览器录像相互独立</p><div class="preflight">${preflightCard("Git / GitHub", preflight.git?.ok, preflight.git?.gh?.account || preflight.git?.gh?.error || "未认证")}${preflightCard("Codex CLI", preflight.codex?.ok, preflight.codex?.version || preflight.codex?.error)}${preflightCard("Claude Docker", preflight.claude?.ok, preflight.claude?.image_id || preflight.claude?.image)}${preflightCard("浏览器页面录像", preflight.browserRecording?.ok, preflight.browserRecording?.ok ? "Chrome · Playwright · 1280×720" : "需要安装浏览器录像依赖")}</div></div>
      ...
      ${setting("claude_model", "Claude 开发模型", settings.claude_model, "仅用于 A/B 开发")}${setting("claude_image", "Claude 容器镜像", settings.claude_image, "从现系统部署配置读取")}${setting("task_generation_max_parallel", "出题并发", settings.task_generation_max_parallel, "与开发并发分开")}${setting("task_pool_min_ready", "题库低水位", settings.task_pool_min_ready, "低于此值自动补题")}${setting("task_pool_target_ready", "题库目标水位", settings.task_pool_target_ready, "每轮补到此数量")}${setting("task_category_weight_backend", "纯后端权重", settings.task_category_weight_backend ?? 15, "默认 15%")}${setting("task_category_weight_frontend", "纯前端权重", settings.task_category_weight_frontend ?? 40, "默认 40%")}${setting("task_category_weight_fullstack", "全栈权重", settings.task_category_weight_fullstack ?? 45, "默认 45%")}${setting("max_pairs_parallel", "Pair 并发", settings.max_pairs_parallel, "同时保留的项目数量", 'type="number" min="1" max="4"')}${setting("max_claude_terminals", "开发终端并发", settings.max_claude_terminals ?? 3, "两个 Pair 共用终端名额；默认最多 3 个，空位后自动启动等待侧", 'type="number" min="1" max="8"')}${setting("ab_prompt_stagger_seconds", "A/B 题面发送间隔", settings.ab_prompt_stagger_seconds ?? 30, "默认先发送 A，30 秒后发送 B；两侧分别从实际发送时刻计时", 'type="number" min="0" max="300"')}${setting("git_author_name", "Git 提交人", settings.git_author_name, "默认刘昱")}${setting("git_author_email", "Git 提交邮箱", settings.git_author_email, "创建远端仓库前必填")}${setting("github_owner", "GitHub Owner", settings.github_owner, "留空时使用 gh 当前账号")}${setting("github_visibility", "仓库可见性", settings.github_visibility, "SOLO-QA 必须能够公开读取，默认 public")}
...
async function renderExports() {
  ...
  $("#content").innerHTML = `<div class="card"><div class="toolbar export-heading"><div><h2>完成 Pair 导出与正式提交</h2><p class="sub">本期 Pair-wise 表单：一道题、双轨迹、双产物、双录像和一份 GSB 对比理由</p><div class="helper-status">${helperStatus}</div></div><div class="toolbar-group"><b>已选 ${state.exportSelection.size} 项 · 待返修 ${repairableSelected} 项</b><button class="secondary" onclick="syncSoloQa()" ${state.helperReady ? "" : "disabled"}>同步全部状态</button><button class="secondary" onclick="preflightSelected()" ${state.exportSelection.size ? "" : "disabled"}>提交前检查</button><button class="secondary" onclick="selectReady()">只选可提交项</button><button class="secondary" onclick="selectNeedsFix()">只选待返修项</button><button class="secondary" onclick="exportSelected()" ${state.exportSelection.size ? "" : "disabled"}>导出 Excel</button><button class="primary" onclick="submitSelected()" ${state.exportSelection.size && state.helperReady && !state.soloQaSubmitRunning ? "" : "disabled"}>${state.soloQaSubmitRunning ? "提交中…" : "提交到 SOLO-QA"}</button><button class="primary repair-submit" onclick="repairRemoteSelected()" ${repairableSelected && state.helperReady ? "" : "disabled"}>批量返修提交</button><button class="danger" onclick="hideSelected()" ${state.exportSelection.size ? "" : "disabled"}>从列表隐藏</button></div></div>
    ${filterBar("exports", [["q", "search", "Pair 唯一 ID、项目编号、题目或题面", filters.q], ["task_type", "select", "全部任务类型", filters.task_type, [["zero_to_one", "0–1"], ["feature", "Feature 迭代"], ["bugfix", "Bug 修复"]]], ["project_category", "select", "全部系统类型", filters.project_category, ["纯后端", "纯前端", "全栈"]], ["difficulty", "select", "全部难度", filters.difficulty, ["简单", "中等", "困难", "地狱"]], ["readiness", "select", "全部资料状态", filters.readiness, [["ready", "可提交"], ["blocked", "待补资料"]]], ["submission_status", "select", "全部提交状态", filters.submission_status, [["ready_to_submit", "待提交"], ["submitting", "提交中"], ["qc_pending", "质检中"], ["qc_passed", "质检通过"], ["needs_fix", "待返修"], ["discarded", "已废弃"], ["failed", "提交失败"], ["needs_review", "待重新确认"]]], ["date_from", "date", "完成日期从", filters.date_from], ["date_to", "date", "完成日期到", filters.date_to], ["include_hidden", "select", "未隐藏记录", filters.include_hidden, [["1", "包含已隐藏"]]]])}
...
async function showTask(id) { try { const task = await api(`/api/tasks/${encodeURIComponent(id)}`); showDialog(`<h2>${esc(task?.title || id)}</h2>${task?.locked_by ? pairIdentity(task.locked_by) : pairIdentity("")}<div class="kv"><b>任务类型</b><span>${taskTypeBadge(task?.task_type)}</span></div><div class="kv"><b>系统类型</b><span>${projectCategoryBadge(task?.project_category)}</span></div><div class="kv"><b>难度</b><span>${esc(task?.difficulty)}</span></div><div class="kv"><b>状态</b><span>${badge(task?.status)}</span></div>${taskPromptHtml(task)}<div class="kv"><b>未通过原因</b><span>${esc(task?.rejection_reason || "—")}</span></div>`); } catch (error) { notify(error.message, true); } }
async function showPair(id) { try { const data = await api(`/api/pairs/${id}`), repo = data.repository, review = data.difficulty_review; const difficulty = review ? `<h3>实际难度复评</h3><div class="detail-grid"><div class="detail-item"><label>出题难度</label><strong>${esc(review.original_difficulty)}</strong></div><div class="detail-item"><label>A / B 实际难度</label><strong>${esc(review.a_difficulty || "复评中")} / ${esc(review.b_difficulty || "复评中")}</strong></div><div class="detail-item"><label>最终实际难度</label><strong>${esc(review.assessed_difficulty || "复评中")} ${badge(review.status)}</strong></div><div class="detail-item"><label>复评依据</label><span>${esc(review.reason || review.error || "正在读取真实改动、轨迹和 Docker 验收证据")}</span></div></div>` : ""; showDialog(`<h2>${esc(data.task.title)}</h2>${pairIdentity(data.id)}<p>${projectCategoryBadge(data.task.project_category)} ${taskTypeBadge(data.task.task_type)} ${badge(data.status)} ${badge(data.stage)}</p><div class="detail-grid"><div class="detail-item"><label>项目编号</label><strong class="code">${esc(data.chain_id)}</strong></div><div class="detail-item"><label>共同基线</label><strong class="code">${esc(data.baseline_sha || "待创建")}</strong></div><div class="detail-item"><label>远端仓库</label><strong class="code">${esc(repo?.remote_url || "待创建")}</strong></div><div class="detail-item"><label>分支</label><strong>main / A / B</strong></div></div><div class="toolbar"><span>当前难度：${esc(data.difficulty_review?.assessed_difficulty || data.task.difficulty)}</span>${difficultyEditButton(data.id)}</div>${pairRetryControls(data)}${taskPromptHtml(data.task)}<h3>A/B 执行</h3>${table(data.arms, [["Arm", (row) => row.arm], ["SessionID", (row) => `<span class="code">${esc(row.session_id)}</span>`], ["PromptID", (row) => `<span class="code">${esc(row.prompt_id)}</span>`], ["状态", (row) => badge(row.status)], ["提交", (row) => `<span class="code">${esc(row.commit_sha)}</span>`]])}${difficulty}<div style="margin-top:18px">${pairButtons(data)}</div>`); } catch (error) { notify(error.message, true); } }
function difficultyEditButton(id) {
  return id ? `<button type="button" class="tiny" onclick="editPairDifficulty('${id}')">编辑难度</button> ` : "";
}
async function editPairDifficulty(id) {
  try {
    const data = await api(`/api/pairs/${id}`), review = data.difficulty_review, delivery = data.delivery || {};
    const locked = Boolean(delivery.remote_id || delivery.submitted_at || ["submitting", "submitted", "qc_pending", "qc_passed", "needs_fix"].includes(delivery.status));
    const current = review?.assessed_difficulty || data.task.difficulty;
    showDialog(`<h2>编辑难度 · ${esc(data.task.title)}</h2>${pairIdentity(id)}<p class="sub">${review ? '修改最终难度，保留原始 A/B 复评证据并记录人工修改。' : '修改出题难度；开发完成后仍会按实际产物复评。'} 简单、中等仍不满足当前提交门槛。</p>${locked ? '<p class="bad">已提交平台的数据不能编辑难度。</p>' : ''}<label>难度<select id="edit-pair-difficulty" ${locked ? 'disabled' : ''}>${['简单','中等','困难','地狱'].map(d => `<option ${d === current ? 'selected' : ''}>${d}</option>`).join('')}</select></label><label>修改说明（选填）<textarea id="edit-difficulty-note" rows="3" maxlength="800" placeholder="填写人工判断的依据"></textarea></label><div class="toolbar"><button type="button" class="primary" ${locked || review?.status === 'running' ? 'disabled' : ''} onclick="savePairDifficulty('${id}',this)">保存难度</button><button type="button" onclick="showPair('${id}')">返回项目</button></div>${review?.status === 'running' ? '<p>正在自动复评，请等复评完成后再编辑。</p>' : ''}`);
  } catch (error) { notify(error.message, true); }
}
async function savePairDifficulty(id, button) {
  button.disabled = true;
  try {
    await api(`/api/pairs/${id}/difficulty`, {method:'POST', body:JSON.stringify({difficulty:$('#edit-pair-difficulty').value, note:$('#edit-difficulty-note').value})});
    await render(); await showPair(id); notify('难度已保存，列表和导出数据已更新');
  } catch (error) { notify(error.message, true); button.disabled = false; }
}
function taskPromptHtml(task) {
  return `<section class="full-prompt"><div class="toolbar"><h3>完整题目</h3><button type="button" class="tiny" onclick="copyFullPrompt()">复制完整题目</button></div><pre id="full-task-prompt" class="prompt-body">${esc(task?.prompt || "暂无题面")}</pre></section>`;
}
async function copyFullPrompt() {
  try { await navigator.clipboard.writeText($("#full-task-prompt").textContent); notify("完整题目已复制"); }
  catch (error) { notify(`复制失败：${error.message}`, true); }
}
function pairRetryControls(data) {
  const delivery = data.delivery || {};
  const locked = Boolean(delivery.remote_id || delivery.submitted_at || ["submitting", "submitted", "qc_pending", "qc_passed", "needs_fix"].includes(delivery.status));
  const stages = new Set(["development", "manual_queue", "ready_to_start", "completed", "cancelled", "development_failed", "replaced", "replacement_failed", "difficulty_rejected", "artifact_failed", "recording_failed", "artifact_failed_evaluated"]);
  return `<section class="pair-retry-controls"><div class="toolbar"><h3>项目重置与单侧排队</h3><button type="button" class="secondary" ${locked ? "disabled" : ""} onclick="resetPairRetries('${data.id}', this)">重置失败次数</button></div><p class="sub">项目累计失败 ${Number(data.development_failure_count || 0)} 次。重置会清零项目及 A/B 失败次数，保留代码和轨迹；正在运行的一侧继续运行。停止的项目由你选择 A 或 B 排队。</p>${locked ? '<p class="sub">该数据已提交平台，保留原交付，不能重新排队。</p>' : ''}<div class="retry-arms">${["A", "B"].map((name) => {
    const arm = (data.arms || []).find((item) => item.arm === name) || {};
    const queued = arm.status === "waiting_terminal_slot";
    const canQueue = !locked && data.baseline_sha && stages.has(data.stage) && ["failed", "completed", "manual_waiting", "queued"].includes(arm.status);
    return `<div class="detail-item"><strong>${name} 侧 ${badge(arm.status || "missing")}</strong><p>开发失败 ${Number(arm.error_retry_count || 0)} 次 · API 错误 ${Number(arm.api_retry_count || 0)} 次</p><button type="button" class="secondary" ${canQueue ? "" : "disabled"} onclick="queuePairArm('${data.id}', '${name}', this)">${queued ? name + ' 已排队' : '单独排队 ' + name}</button></div>`;
  }).join("")}</div><p class="sub">排队后按原题和共同基线开启新会话，旧记录归档；只重跑所选侧，继续遵守终端并发和 API 冷却。</p></section>`;
}
async function resetPairRetries(id, button) {
  button.disabled = true;
  try {
    await api(`/api/pairs/${id}/reset-retries`, {method: "POST", body: "{}"});
    await render(); await showPair(id); notify("失败次数已清零，可以分别排队 A 或 B；运行中的会话不受影响");
  } catch (error) { notify(error.message, true); button.disabled = false; }
}
async function queuePairArm(id, arm, button) {
  if (!window.confirm(`将 ${id} 的 ${arm} 侧从共同基线重新排队。该侧旧代码和轨迹会归档，现有验收、录像及双方 GSB 需按新产物更新；另一侧保留。是否继续？`)) return;
  button.disabled = true;
  try {
    const result = await api(`/api/pairs/${id}/arms/${arm}/queue`, {method: "POST", body: "{}"});
    notify(`正在保存 ${arm} 侧旧记录并准备排队`);
    await poll(result.operationId, async () => { await render(); await showPair(id); notify(`${arm} 侧已排队，空出终端名额后启动`); });
  } catch (error) { notify(error.message, true); }
...
async function confirmGsb(id) { const verdict = $('input[name="verdict"]:checked')?.value, aReason = $("#gsb-a-reason").value, bReason = $("#gsb-b-reason").value, confirmedBy = $("#confirmed-by").value, button = $("#gsb-confirm"), errorBox = $("#gsb-save-error"); if (button) button.disabled = true; if (errorBox) errorBox.textContent = "正在保存…"; try { await api(`/api/pairs/${id}/gsb/confirm`, { method: "POST", body: JSON.stringify({ verdict, aReason, bReason, confirmedBy }) }); $("#dialog").close(); notify("GSB 已人工确认，记录进入待正式提交状态"); render(); } catch (error) { if (errorBox) errorBox.textContent = `保存失败：${error.message}`; notify(error.message, true); } finally { if ($("#dialog")?.open && button) button.disabled = false; } }
...
async function saveSettings() { const body = {}; $$('[data-setting]').forEach((input) => { body[input.dataset.setting] = ["task_generation_max_parallel", "task_pool_min_ready", "task_pool_target_ready", "task_category_weight_backend", "task_category_weight_frontend", "task_category_weight_fullstack", "max_pairs_parallel", "max_claude_terminals", "ab_prompt_stagger_seconds"].includes(input.dataset.setting) ? Number(input.value) : input.value; }); try { await api("/api/settings", { method: "POST", body: JSON.stringify(body) }); notify("设置已保存，新作业将使用这些值"); render(); } catch (error) { notify(error.message, true); } }
