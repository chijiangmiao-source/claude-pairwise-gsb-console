...
const pages = {
  ...
  tasks: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><div class="toolbar"><div><h2>题目池</h2><p class="sub">0–1/Feature 需困难或地狱；Bug 修复最低可为中等</p></div><div class="toolbar-group"><button class="secondary" onclick="importTasks()">导入历史困难题</button>${automationAction}</div></div>
      <div class="automation-strip ${automation.enabled ? "enabled" : ""}"><div><strong>${automation.enabled ? "完整流程持续运行" : "完整流程尚未启动"}</strong><small>${automation.enabled ? "现有合格题按入库顺序使用；出现 429 时暂停新终端，冷却后每次只恢复一侧" : "点击后自动完成仓库准备、A/B 开发、Docker 验收、录像和 GSB"}</small></div><div class="automation-metrics"><span>活动 Pair <b>${automation.activePairs}/${automation.targetPairs}</b></span>${automation.waitingApiPairs ? `<span>API 排队 Pair <b>${automation.waitingApiPairs}</b></span>` : ""}${automation.waitingApiArms ? `<span>等待重试侧 <b>${automation.waitingApiArms}</b></span>` : ""}${automation.apiCooldownUntil ? `<span>API 冷却至 <b>${esc(new Date(automation.apiCooldownUntil).toLocaleTimeString("zh-CN", {hour12:false}))}</b></span>` : ""}<span>可用题目 <b>${automation.readyTasks}</b></span><span>选题方式 <b>有题即用</b></span><span>${esc(stageText)}</span></div></div>
  ...
  settings: async () => {
    ...
    $("#content").innerHTML = `<div class="card"><h2>运行环境预检</h2><p class="sub">Git、Codex、Claude 容器和浏览器录像相互独立</p><div class="preflight">${preflightCard("Git / GitHub", preflight.git?.ok, preflight.git?.gh?.account || preflight.git?.gh?.error || "未认证")}${preflightCard("Codex CLI", preflight.codex?.ok, preflight.codex?.version || preflight.codex?.error)}${preflightCard("Claude Docker", preflight.claude?.ok, preflight.claude?.image_id || preflight.claude?.image)}${preflightCard("浏览器页面录像", preflight.browserRecording?.ok, preflight.browserRecording?.ok ? "Chrome · Playwright · 1280×720" : "需要安装浏览器录像依赖")}</div></div>
      ...
      ${setting("claude_model", "Claude 开发模型", settings.claude_model, "仅用于 A/B 开发")}${setting("claude_image", "Claude 容器镜像", settings.claude_image, "从现系统部署配置读取")}${setting("task_generation_max_parallel", "出题并发", settings.task_generation_max_parallel, "与开发并发分开")}${setting("task_pool_min_ready", "题库低水位", settings.task_pool_min_ready, "低于此值自动补题")}${setting("task_pool_target_ready", "题库目标水位", settings.task_pool_target_ready, "每轮补到此数量")}${setting("task_category_weight_backend", "纯后端权重", settings.task_category_weight_backend ?? 15, "默认 15%")}${setting("task_category_weight_frontend", "纯前端权重", settings.task_category_weight_frontend ?? 40, "默认 40%")}${setting("task_category_weight_fullstack", "全栈权重", settings.task_category_weight_fullstack ?? 45, "默认 45%")}${setting("max_pairs_parallel", "Pair 并发", settings.max_pairs_parallel, "每个 Pair 使用 A/B 两个终端，最多 4 个 Pair（共 8 个终端）", 'type="number" min="1" max="4"')}${setting("ab_prompt_stagger_seconds", "A/B 题面发送间隔", settings.ab_prompt_stagger_seconds ?? 30, "默认先发送 A，30 秒后发送 B；两侧分别从实际发送时刻计时", 'type="number" min="0" max="300"')}${setting("git_author_name", "Git 提交人", settings.git_author_name, "默认刘昱")}${setting("git_author_email", "Git 提交邮箱", settings.git_author_email, "创建远端仓库前必填")}${setting("github_owner", "GitHub Owner", settings.github_owner, "留空时使用 gh 当前账号")}${setting("github_visibility", "仓库可见性", settings.github_visibility, "SOLO-QA 必须能够公开读取，默认 public")}
