...
Claude 开发遇到 429、504、证书或连接类 API Error 时会自动归档现场并计入 Pair 级失败额度。第一次进入“等待 API 自动重试”；A/B 合计第二次出现任何开发或 API 错误时只停止本次失败侧，另一侧继续到完成或触发从自身题面发送时间计算的 75 分钟无业务代码规则，随后再退役 Pair 并换下一道合格 0–1 题。
...
命令画面。待两侧录像结束后，系统根据真实轨迹和验收结果生成 GSB 与提交数据。首轮题面发送后 15 分钟仍无业务代码会告警；重跑侧到 40 分钟仍无代码且活动轨迹与上次完全相同时提前终止，轨迹不同则继续运行；75 分钟仍无业务代码会
终止本次开发。A/B 容器都准备完成后先
