...
Claude 开发遇到 429、504、证书或连接类 API Error 时会自动归档现场并显示“等待 API 自动重试”。这类等待不计开发失败，也不会新建额外 Pair；冷却到期后系统从共同基线开启全新首轮 Session，并只发送一次原题面。
...
命令画面。待两侧录像结束后，系统根据真实轨迹和验收结果生成 GSB 与提交数据。首轮题面发送后 15 分钟仍无业务代码会告警，40 分钟仍无业务代码会
